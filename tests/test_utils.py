...
def test_zac_tags2zabbix_tags(tags: set[tuple[str, str]], expected: list[HostTag]):
...
def test_mapping_values_with_prefix(hostgroup_map_file: Path, prefix: str):
    # NOTE: feels kind of odd to manually construct MapFile here, but whatever.
    m = MapFile(path=hostgroup_map_file, name="Test map file").read()
